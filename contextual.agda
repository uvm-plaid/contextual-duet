...
mutual
  data ⟨_,_⟩∈𝒱⟦_ː_⟧ : 𝓋 → 𝓋 → Sens → 𝓋τ ᴢ → Set where
    ...
    -- 𝒱λ : ∀ {e₁ e₂ γ₁ γ₂ v₁ v₂ τ₁ τ₂ s s′ s′′ s₁ s₂ Σ Σ′}
    --   →  ∃ Γ ST ⟨ γ₁ , γ₂ ⟩∈𝒢⟦ Σ′ ː Γ ⟧
    --   → ⟨ v₁ , v₂ ⟩∈𝒱⟦ s′′ ː τ₁ ⟧
    --   → ⟨ (v₁ ∷ γ₁) ⊢ e₁ , (v₂ ∷ γ₂) ⊢ e₂ ⟩∈ℰ⟦ (s + (Σ ⨰ Σ′) + (s′ × s′′)) ː (cut [ s′′ ] τ₂) ⟧ -- need subst for 𝓋τ
    --   ---------------------------------------------------------------------------------
    --   → ⟨ ƛ⦂ e₁ ∥ γ₁ , ƛ⦂ e₂ ∥ γ₂ ⟩∈𝒱⟦ s ː ƛ⦂ τ₁ ⇒[ (s′ + Σ ⨰ Σ′ ) ∔ [] ] τ₂ ⟧

  ⟨_⊢_,_⊢_⟩∈ℰ⟦_ː_⟧ : γ[ ᴢ ] → Term ᴢ → γ[ ᴢ ] → Term ᴢ → Sens → 𝓋τ ᴢ → Set
...
fp : ∀ {Γ ℾ e τ Σ γ₁ γ₂ Σ′} → Γ ⊢ e ⦂ τ , Σ → ⟨ γ₁ , γ₂ ⟩∈𝒢⟦ Σ′ ː ℾ ⟧ → ⟨ γ₁ ⊢ e , γ₂ ⊢ e ⟩∈ℰ⟦ Σ ⨰ Σ′ ː (Σ′ ⟨⟨ τ ⟩⟩) ⟧
fp a b = {!   !}
