...
module _ {ℓ} {A : Set ℓ} {{_ : has[+] A}} {{_ : cor[+] A}} {{_ : has[≡?] A}} where
  ...
  _⊔[qty]_ : {{_ : has[⊔] A}} → qty A → qty A → qty A
  _ ⊔[qty] `∞ = `∞
  `∞ ⊔[qty] _ = `∞
  ⟨ x ⟩ ⊔[qty] ⟨ y ⟩ = ⟨ x ⊔ y ⟩
...
mutual
  data 𝓋 : ℕ → Set where
    ...
    inl_ : ∀ {N} → 𝓋 N → 𝓋 N
    inr_ : ∀ {N} → 𝓋 N → 𝓋 N
    _pair_ : ∀ {N} → 𝓋 N → 𝓋 N → 𝓋 N
    ƛ⦂_∥_ : ∀ {N} → Term (ꜱ N) → γ[ N ] → 𝓋 N
    𝒷_ : ∀ {N} → 𝔹 → 𝓋 N
    𝓇_ : ∀ {N} → 𝔽 → 𝓋 N
...
data _⊢_⇓_ : ∀ {N} → γ[ N ] → Term N → 𝓋 N → Set where
  ...
  -- INL
  ⊢`inl : ∀ {N} {γ : γ[ N ]} {e : Term N} {𝓋 : 𝓋 N} {τ₂ : τ N}
    → γ ⊢ e ⇓ 𝓋
    --------------------------------------------------
    → γ ⊢ (inl τ₂ ∥ e) ⇓ inl 𝓋

  -- INR
  ⊢`inr : ∀ {N} {γ : γ[ N ]} {e : Term N} {𝓋 : 𝓋 N} {τ₁ : τ N}
    → γ ⊢ e ⇓ 𝓋
    --------------------------------------------------
    → γ ⊢ (inr τ₁ ∥ e) ⇓ inr 𝓋

  -- CASE-LEFT
  -- ⊢`case/l : ∀ {N} {γ : γ[ N ]} {e₁ : Term N} {e₂ e₃ : Term (ꜱ N)} {𝓋₁ 𝓋₂ : 𝓋 N}
  --   → γ ⊢ e₁ ⇓ inl 𝓋₁
  --   → (𝓋₁ ∷ γ) ⊢ e₂ ⇓ 𝓋₂
  -- ------------------------------------------------------------------------------------
  --   → γ ⊢ case e₁ of e₂ ∥ e₃ ⇓ 𝓋₂
  ...
  -- CASE-RIGHT
  -- ⊢`case/r : ∀ {N} {γ : γ[ N ]} {i : idx (ꜱ N)} {e₁ : Term N} {e₂ e₃ : Term (ꜱ N)} {𝓋₁ 𝓋₂ : 𝓋 N}
  --   → γ ⊢ e₁ ⇓ inl 𝓋₁
  --   → (𝓋₁ ∷ γ) ⊢ e₃ ⇓ 𝓋₂
  -- ------------------------------------------------------------------------------------
  --   → γ ⊢ case e₁ of e₂ ∥ e₃ ⇓ 𝓋₂
  ...
  -- IF-TRUE
  ⊢`if-true : ∀ {N} {γ : γ[ N ]} {e₁ e₂ e₃ : Term N} {𝓋 : 𝓋 N}
    → γ ⊢ e₁ ⇓ 𝒷 ɪ
    → γ ⊢ e₂ ⇓ 𝓋
  ------------------------------------------------------------------------------------
    → γ ⊢ if e₁ ∥ e₂ ∥ e₃ ⇓ 𝓋
  ...
  -- IF-FALSE
  ⊢`if-false : ∀ {N} {γ : γ[ N ]} {e₁ e₂ e₃ : Term N} {𝓋 : 𝓋 N}
    → γ ⊢ e₁ ⇓ 𝒷 ᴏ
    → γ ⊢ e₃ ⇓ 𝓋
  ------------------------------------------------------------------------------------
    → γ ⊢ if e₁ ∥ e₂ ∥ e₃ ⇓ 𝓋
  ...
  -- LET
  -- ⊢`let : ∀ {N} {γ : γ[ N ]} {e₁ : Term N} {e₂ : Term (ꜱ N)} {𝓋₁ 𝓋₂ : 𝓋 N}
  --   →  γ ⊢ e₁ ⇓ 𝓋₁
  --   →  (𝓋₁ ∷ γ) ⊢ e₂ ⇓ 𝓋₂
  --   -----------------------------------------------
  --   → γ ⊢ (`let e₁ ∥ e₂) ⇓ 𝓋₂
  ...
  -- APP
  -- ⊢`app : ∀ {N} {γ γ′ : γ[ N ]} {e′ : Term (ꜱ N)} {e₁ e₂ : Term N} {𝓋₁ 𝓋₂ : 𝓋 N}
  --   → γ ⊢ e₁ ⇓ (ƛ⦂ e′ ∥ γ′ )
  --   → γ ⊢ e₂ ⇓ 𝓋₁
  --   → 𝓋₁ ∷ γ′ ⊢ e′ ⇓ 𝓋₂
  --   -----------------------------------------------------------
  ...
  -- PAIR
  ⊢`_pair_ : ∀ {N} {γ : γ[ N ]} {e₁ e₂ : Term N} {𝓋₁ 𝓋₂ : 𝓋 N}
    → γ ⊢ e₁ ⇓ 𝓋₁
    → γ ⊢ e₂ ⇓ 𝓋₂
    -----------------------------------------------------------
    → γ ⊢ e₁ `pair e₂ ⇓ 𝓋₁ pair 𝓋₂
  ...
  -- PROJ1
  ⊢`fst : ∀ {N} {γ : γ[ N ]} {e : Term N} {𝓋₁ 𝓋₂ : 𝓋 N}
    → γ ⊢ e ⇓ 𝓋₁ pair 𝓋₂
  --------------------------------------
    → γ ⊢ fst e ⇓ 𝓋₁

  -- PROJ2
  ⊢`snd : ∀ {N} {γ : γ[ N ]} {e : Term N} {𝓋₁ 𝓋₂ : 𝓋 N}
    → γ ⊢ e ⇓ 𝓋₁ pair 𝓋₂
  -----------------------------------------
    → γ ⊢ fst e ⇓ 𝓋₂
