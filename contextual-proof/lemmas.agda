...
postulate
  ...
  L-P≤KP : ∀ {N r p} → (Σ₁ :  Σₚ[ N ]) → ( Σ₂ Σ′ : Σ[ N ]) → (𝑒^ᴿ (p2r (((([vec]⌉ Σ′ ⌈⸢ ⟨ one ⟩ ⸣) ⨰ Σ₁)) +[qty] ((⌉ Σ₂ ⨰ Σ′ ⌈⸢ ⟨ 1 ⟩ ⸣) ×[qty] p))) × r ) ≤ᵣ ((𝑒^ᴿ (p2r (([vec]⌉ Σ′ ⌈⸢ ⟨ 1 ⟩ ⸣ ⨰ Σ₁) +[qty] (([vec]⌉ Σ′ ⌈⸢ ⟨ 1 ⟩ ⸣ ⨰ [vec]⌉ Σ₂ ⌈⸢ ⟨ 1 ⟩ ⸣) ×[qty] p)) )) × r)
...
postulate
  ...
  L7 : ∀ {N} (Σ′ : Σ[ N ]) (τ₃ τ₂ τ₁ : τ N)
    ...
    → (Σ′ ⟨⟨ τ₁ ⟩⟩) ≡ instantiateΣ/τ (𝐜< N > ⟨ 0 ⟩) τ₂
    → τ₃ ≡ τ₂
  L8 : ∀ {N} {τ₁ ℾ Σ₀ Σ₁ Σ₂ τ₂ s sb} {e : Term (ꜱ N)}
  ...
  delayedΣEq : ∀ {N} {Γ : Γ[ N ]} {e₁ e′₂ τ₁ sx†₁ Σ₀ Σ₁ Σ′₁ τ₂ γ′ γ′₁ Γ₁ τ₁₁ τ₂₁ sb}
