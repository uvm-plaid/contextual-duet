...
mutual
  ...
  data _⊢_⇓ₚ_ : ∀ {N τ} → γ[ N ] → PTerm N → 𝒟 (∃ v ⦂ 𝓋 ST ⊢ v ⦂ τ) → Set where
    ...
    -- PCASE-LEFT
    ⊢`pcase/l : ∀ {N} {γ : γ[ N ]} {e₁ : Term N} {e₂ e₃ : PTerm (ꜱ N)} {𝓋₁ : 𝓋} {τ} {𝓋₂ : 𝒟 (∃ v ⦂ 𝓋 ST ⊢ v ⦂ τ )}
      → γ ⊢ e₁ ⇓ inl 𝓋₁
      → (𝓋₁ ∷ γ) ⊢ e₂ ⇓ₚ 𝓋₂
    ------------------------------------------------------------------------------------
    ...
    -- PCASE-RIGHT
    ⊢`pcase/r : ∀ {N} {γ : γ[ N ]} {e₁ : Term N} {e₂ e₃ : PTerm (ꜱ N)} {𝓋₁ : 𝓋} {τ} {𝓋₂ : 𝒟 (∃ v ⦂ 𝓋 ST ⊢ v ⦂ τ )}
      → γ ⊢ e₁ ⇓ inr 𝓋₁
      → (𝓋₁ ∷ γ) ⊢ e₃ ⇓ₚ 𝓋₂
    ------------------------------------------------------------------------------------
      → γ ⊢ pcase e₁ of e₂ ∥ e₃ ⇓ₚ 𝓋₂
    ...
    -- BIND
    -- ⊢`bind : ∀ {N} {γ : γ[ N ]} {e₂ : PTerm (ꜱ N)} {e₁ : PTerm N} {τ₁ τ₂} {𝓋₁ : 𝒟 (∃ v₁ ⦂ 𝓋 ST ⊢ v₁ ⦂ τ₁ )}
    --   → γ ⊢ e₁ ⇓ₚ 𝓋₁
    --   → (F : ∀ (v₁ ∈ 𝓋₁) → ∃ v₂ ST v₁ ∷ γ ⊢ e₂ ⇓ₚ v₂)
    --   → let v₃ = do ⟨∃ v₁ , ⊢v₁ ⟩ ← 𝓋₁ ; dπ₁ (F v₁) in
    --   -----------------------------------------------------------
    --   γ ⊢ (`bind e₁ ∥ e₂) ⇓ₚ v₃
    ...
    --   → γ ⊢ e₁ ⇓ₚ return ⟨∃ v₁ , ⊢τ ⟩
    --   → (F : ∃ v₂ ST v₁ ∷ γ ⊢ e₂ ⇓ₚ v₂)
    --   → let v₃ = do ⟨∃ v₁ , ⊢v₁ ⟩ ← (return ⟨∃ v₁ , ⊢τ ⟩) ; dπ₁ F in
    --   -----------------------------------------------------------
    --   γ ⊢ (`bind e₁ ∥ e₂) ⇓ₚ v₃
