...
postulate
  ...
  ⟦_ː_⟧ˢ[_ː_] : ∀ {N} {Γ : Γ[ N ]} {ℾ Σ₀ e τ Σ Σ′} → (e : Term N) → (⊢e : Γ , Σ₀ ⊢ e ⦂ τ , Σ) → (γ : γ[ N ]) → ℾ ⊢ γ → ∃ v ⦂ 𝓋 ST ⊢ v ⦂  Σ′ ⟨⟨ τ ⟩⟩
  corr : ∀ {N} {Γ : Γ[ N ]} {ℾ Σ₀ e τ Σ Σ′ v ⊢v} → (e : Term N) → (⊢e : (Γ , Σ₀ ⊢ e ⦂ τ , Σ )) → (γ : γ[ N ]) → (⊢γ : ℾ ⊢ γ) → γ ⊢ e ⇓ v → ⟦ e ː ⊢e ⟧ˢ[ γ ː ⊢γ ] ≡ ⟨∃ v , ⊢v ⟩

-- privacy evaluation
⟦_ː_⟧ᵖ[_ː_] : ∀ {N} {Γ : Γ[ N ]} {ℾ Σ₀ e τ Σ Σ′} → (e : PTerm N) → Γ , Σ₀ ⊢ₚ e ⦂ τ , Σ → (γ : γ[ N ]) → ℾ ⊢ γ → 𝒟 (∃ v ⦂ 𝓋 ST ⊢ v ⦂ Σ′ ⟨⟨ τ ⟩⟩)
⟦ _`papp_ e₁ e₂ ː ⊢`papp δ₁ δ₂ ⟧ᵖ[ γ ː ⊢γ ]
  with ⟦ e₁ ː δ₁ ⟧ˢ[ γ ː ⊢γ ] | ⟦ e₂ ː δ₂ ⟧ˢ[ γ ː ⊢γ ]
... | ⟨∃  (pƛ⦂ e′ ∥ γ′) , (⊢pλ ⊢γ′ ⊢e′ i₁ i₂ pz) ⟩ | ⟨∃ v , ⊢v ⟩ = ⟦ {! e′  !} ː {!   !} ⟧ᵖ[ {!   !} ː {!   !} ]


-- let ⟨∃  (pƛ⦂ e′ ∥ γ′) , (⊢pλ ⊢γ′ ⊢e′ i₁ i₂ pz) ⟩ = ⟦ e₁ ː δ₁ ⟧ˢ[ γ ː ⊢γ ]
--       ⟨∃ v , ⊢v ⟩ = ⟦ e₂ ː δ₂ ⟧ˢ[ γ ː ⊢γ ]
--   in ⟦ e′ ː ⊢e′ ⟧ᵖ[ (v ∷ γ′) ː (⊢s ⊢v ⊢γ′) ]
