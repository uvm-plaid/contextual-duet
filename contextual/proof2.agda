...
fpc : ∀ {Γ : Γ[ ᴢ ]} {ø : γ[ ᴢ ]} {e : Term ᴢ} {τ : τ ᴢ} {ℾ} → ℾ ⊢ ø → ℾ ⊢ ø → Γ ⊢ e ⦂ τ , zero → ⟨ ø , ø ⟩∈𝒢⟦ zero ː ℾ ⟧ → ⟨ ø ⊢ e , ø ⊢ e ⟩∈ℰ⟦ zero ː (zero ⟨⟨ τ ⟩⟩) ⟧
...
sound : ∀ {Γ : Γ[ ᴢ ]} {ø : γ[ ᴢ ]} {e s s′ r₁ r₂ r′₁ r′₂ ℾ}
  → ℾ ⊢ ø → ℾ ⊢ ø → ⟨ ø , ø ⟩∈𝒢⟦ zero ː ℾ ⟧
  → Γ ⊢ e ⦂ (ƛ⦂ ℝT ⇒[ zero ∔ [ s ] ] ℝT) , zero
  → ∣ r₁ - r₂ |ᴺ≤ s′
  → ø ⊢ e `app (`ℝ r₁) ⇓ 𝓇 r′₁
  → ø ⊢ e `app (`ℝ r₂) ⇓ 𝓇 r′₂
  → ∣ r′₁ - r′₂ |ᴺ≤ (s × s′)
sound {r′₁ = r′₁} {r′₂} ⊢ø₁ ⊢ø₂ r[⊢ø₁,⊢ø₂] ⊢e ε (⊢`app {γ = γ₁} {e′ = e′₁} δ₁ δ₂ δ₃) (⊢`app {γ = γ₂} {e′ = e′₂} δ₄ δ₅ δ₆)
  = fp ⊢ø₁ ⊢ø₂ ⊢e r[⊢ø₁,⊢ø₂] (ƛ⦂ e′₁ ∥ γ₁ ) (ƛ⦂ e′₂ ∥ γ₂ ) (typeSafety ⊢e δ₁) (typeSafety ⊢e δ₄) ⟨ δ₁ , δ₄ ⟩
