...
mutual
  ...
  ⟨ .(ƛ⦂ e₁ ∥ γ₁) , .(ƛ⦂ e₂ ∥ γ₂) ⟩∈𝒱′⟦ ƛ⦂ τ₁ ⇒[ s† ∔ [ sx† ] ] τ₂
                                 ː ⊢λ {N = N₁} {ℾ = ℾ₁} {e = e₁} {γ = γ₁} {s = sx†₁} {s′ = s†₁} {Σ₁ = Σ₁} H₁₁ H₁₂ H₁₃ H₁₄ H₁₅
                                 , ⊢λ {N = N₂} {ℾ = ℾ₂} {e = e₂} {γ = γ₂} {s = sx†₂} {s′ = s†₂} {Σ₁ = Σ₂} H₂₁ H₂₂ H₂₃ H₂₄ H₂₅
                                 ː s ⟧ =
    ∃ ε ⦂ N₁ ≡ N₂ ST
    ...
    → ⟨ v₁ , v₂ ⟩∈𝒱′⟦ τ₁ ː ε₁ , ε₂ ː s′ ⟧
    → ⟨ (v₁ ∷ subst[(λ N → γ[ N ] )] ε γ₁) ⊢ subst[(λ N → Term (ꜱ N) )] ε e₁ , (v₂ ∷ γ₂) ⊢ e₂ ⟩∈ℰ⟦ s + (Σ₂ ⨰ Σ′) {- or s†   -} + (sx† × s′) ː (substSx/τ s′ τ₂) ⟧))
