...
postulate
  ...
  L15 : ∀ {N} {γ₁ γ₂ : γ[ N ]} {ℾ : ℾ[ N ]} {Γ₁ Γ₂} {Σ₁ : Σ[ N ]} → ℾ ⊢ γ₁ → ℾ ⊢ γ₂
    ...
    → mapⱽ (instantiateΣ/τ Σ₁) Γ₂ ⊢ γ₂
    → Γ₁ ≡ Γ₂
  L15′ : ∀ {N} {γ₁ γ₂ : γ[ N ]} {ℾ : ℾ[ N ]} {Γ} {Σ : Σ[ N ]} → ℾ ⊢ γ₁ → ℾ ⊢ γ₂
    → mapⱽ (instantiateΣ/τ (𝐜< N > ⟨ 0 ⟩)) Γ ⊢ γ₁
    → mapⱽ (instantiateΣ/τ (𝐜< N > ⟨ 0 ⟩)) Γ ⊢ γ₂
  ...
  L19 : ∀ {N} (Σ′ : Σ[ N ]) (τ₃ τ₂ τ₁ : τ N)
    ...
    → (Σ′ ⟨⟨ τ₁ ⟩⟩) ≡ instantiateΣ/τ (𝐜< N > ⟨ 0 ⟩) τ₂
    → τ₃ ≡ τ₂
  L20 : ∀ {N} {τ₁ ℾ τ₂} {s s₂ s₁ : Sens} {Σ₁₁′ Σ₁₂′ Σ₂₁′ Σ₂₂′ : Σ[ N ]} {e : Term (ꜱ N)} → ⇧ᵗ< ᴢ > τ₁ ∷ mapⱽ ⇧ᵗ< ᴢ > ℾ ⊢ e ⦂ τ₂ , (s × s₁) ∷ (Σ₁₁′ + (s ⨵ Σ₂₁′))
    → ⇧ᵗ< ᴢ > τ₁ ∷ mapⱽ ⇧ᵗ< ᴢ > ℾ ⊢ e ⦂ τ₂ , (s × s₂) ∷ (Σ₁₂′ + (s ⨵ Σ₂₂′))
    → Σ₁₁′ ≡ Σ₁₂′
  ...
  constantΣEq : ∀ {N} {Γ : Γ[ N ]} {e₁ e₂ : Term N} {γ : γ[ N ]} {τ₁ τ₂ : τ N} {τ₂₁} {e₁′ s τ₁′ τ₂′ s′ s₁ ℾ τ₂ } {Σ₁ Σ₁′ Σ₂ Σ₂′ : Σ[ N ]}
    → Γ ⊢ (e₁ `app e₂) ⦂ cut Σ₂ τ₂ , (Σ₁ + (s ⨵ Σ₂))
    → γ ⊢ e₁ ⇓ (ƛ⦂ e₁′ ∥ γ )
    → ⊢ (ƛ⦂ e₁′ ∥ γ ) ⦂ ƛ⦂ τ₁′ ⇒[ s′ ∔ [ s ] ] τ₂′
    → mapⱽ ⇧ᵗ (τ₁ ∷ ℾ) ⊢ e₁′ ⦂ τ₂₁ , (s × s₁) ∷ (Σ₁′ + (s ⨵ Σ₂′))
    → Σ₁ ≡ Σ₁′
