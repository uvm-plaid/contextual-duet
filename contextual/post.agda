...
postulate
  ...
  ▵× : ∀ {N} {r₁₁ r₁₂ r₂₁ r₂₂ : ℕ} {Σ₁ Σ₂ Σ′ : Σ[ N ]} → [vec]⌉ (Σ₁ + Σ₂) ⌈⸢ `∞ ⸣ ⨰ Σ′ ≡ ⟨ 0 ⟩
    ...
    → ⟨ ∣ r₁₂ - r₂₂ ∣ ⟩ ≤ (Σ₂ ⨰ Σ′)
    → ⟨ ∣ r₁₁ ×ᴺ r₁₂ - r₂₁ ×ᴺ r₂₂ ∣ ⟩ ≡< qty ℕ > ⟨ 0 ⟩
  -- ? : ∀ {N} {γ : γ[ N ]} {Σ′} {ℾ₁ : Γ[ N ]} {ℾ₂ : ℾ[ N ]} → mapⱽ (instantiateΣ/τ Σ′) ℾ₁ ⊢ γ → ℾ₂ ⊢ γ → mapⱽ (instantiateΣ/τ Σ′) ℾ₁ ≡ ℾ₂
  -- ? : ∀ {N} {γ : γ[ N ]} {Σ′} {ℾ₁ ℾ₂ : Γ[ N ]} → mapⱽ (instantiateΣ/τ Σ′) ℾ₁ ⊢ γ → mapⱽ (instantiateΣ/τ Σ′) ℾ₂ ⊢ γ → ℾ₁ ≡ ℾ₂
  -- ? : ∀ {N} {γ : γ[ N ]} {ℾ₁ ℾ₂ : ℾ[ N ]} → ℾ₁ ⊢ γ → ℾ₂ ⊢ γ → ℾ₁ ≡ ℾ₂
  L3 : ∀ {N} {γ₁ γ₂ : γ[ N ]} {ℾ : ℾ[ N ]} {Γ₁ Γ₂} {Σ₁ : Σ[ N ]} → ℾ ⊢ γ₁ → ℾ ⊢ γ₂
    → mapⱽ (instantiateΣ/τ Σ₁) Γ₁ ⊢ γ₁
    → mapⱽ (instantiateΣ/τ Σ₁) Γ₂ ⊢ γ₂
    → Γ₁ ≡ Γ₂
  L3′ : ∀ {N} {γ₁ γ₂ : γ[ N ]} {ℾ : ℾ[ N ]} {Γ} {Σ : Σ[ N ]} → ℾ ⊢ γ₁ → ℾ ⊢ γ₂
  ...
  L8 : ∀ {N} {τ₁ ℾ Σ₁ Σ₂ τ₂ s} {e : Term (ꜱ N)}
    ...
    → ℾ ⊢ ƛ⦂ τ₁ ⇒ e ⦂ (ƛ⦂ τ₁ ⇒[ zero ∔ s ∷ Σ₂ ] τ₂) , zero
    → Σ₁ ≡ Σ₂
  L9 : ∀ {N} {γ₁ γ′₁ : γ[ N ]} {e e′} → γ₁ ⊢ e ⇓ (ƛ⦂ e′ ∥ γ′₁) → γ₁ ≡ γ′₁
  L9-1 : ∀ {N} {γ′₁ : γ[ N ]} {e₁ e′₁ γ′₂} {e′₂} →  γ′₁ ⊢ e₁ ⇓ (ƛ⦂ e′₁ ∥ γ′₁) → γ′₂ ⊢ e₁ ⇓ (ƛ⦂ e′₂ ∥ γ′₂) → e′₁ ≡ e′₂
  L10 : ∀ {N} → (Σ₂ Σ′ : Σ[ N ]) → (τ : τ (ꜱ N)) →  (Σ′ ⟨⟨ substΣ/τ ᴢ Σ₂ τ ⟩⟩) ≡ substSx/τ< ᴢ > (Σ₂ ⨰ Σ′) (⇧ᵗ< ᴢ > ((⟨ 0 ⟩ ∷ Σ′) ⟨⟨ τ ⟩⟩))
  ...
  delayedΣEq : ∀ {N} {Γ : Γ[ N ]} {e₁ e′₂ τ₁ sx†₁ Σ₁ Σ′₁ τ₂ γ′₁ Γ₁ τ₁₁ τ₂₁}
