...
... | Σ₁≡Σ₂ = ⟨∃ ↯ , ⟨ ⟨ ⟨ ⟨ ↯ , Σ₁≡Σ₂ ⟩ , ↯ ⟩ , ↯ ⟩  , P ⟩ ⟩
  where
    P : ∀ {v₁ v₂ : 𝓋} {ε₁ : ⊢ v₁ ⦂ (_ ⟨⟨ _ ⟩⟩)} {ε₂ : ⊢ v₂ ⦂ (_ ⟨⟨ _ ⟩⟩)} {s′ Σ′′}
      ...
      → ⟨ v₁ , v₂ ⟩∈𝒱′⟦ _ ⟨⟨ _ ⟩⟩ ː ε₁ , ε₂ ː s′ ⟧
      → ⟨ v₁ ∷ γ₁ ⊢ e₁ , v₂ ∷ γ₂ ⊢ e₁ ⟩∈ℰ⟦ (Σ ⨰ Σ′) + (Σ′ ⨰ Σe₁) + (sx†₂ × s′) ː substSx/τ s′ (⇧ᵗ ((⇧ˢ _ ⟨⟨ _ ⟩⟩))) ⟧
...
... | ↯ | ↯ | ↯ with L15 ⊢γ₁ ⊢γ₂ ⊢γ₁′ ⊢γ₂′
    | L19 Σ′ τ₁₂ τ₁₁ τ₁ ε₂₁ ε₁₁
    | L21-1 r₁ r₄
... | ↯ | ↯ | ↯ with L22 Σ₂ Σ′ τ₂ | ε₁₃ | constantΣEq ((⊢`app {Σ₁ = Σ₁} {Σ₂ = Σ₂} {τ₁ = τ₁} {τ₂} e₁ e₂)) r₁ (⊢λ {ℾ = ℾ₁} {τ₁ = τ₁₁} {τ₂ = τ₂₁} {s = sx†₁} {s′ = s†₁} {Σ₁ = Σ′₁} ⊢γ₁′ ⊢e₁ ε₁₁ ε₁₂ ε₁₃) ⊢e₁
... | ZZ | Z | ↯ with IH′ {v₁ = 𝓋₁} {v₂ = 𝓋₂} {ε₁ = (typeSafety e₂ r₂)} {ε₂ = (typeSafety e₂ r₅)} {s′ = Σ₂ ⨰ Σ′} {Σ′ = Σ′} (L15′ ⊢γ₁ ⊢γ₂ ⊢γ₁′ ⊢γ₂′) IH₂ v₁ v₂ (subst[( λ X → ⊢ v₁ ⦂ X )] ZZ ε₁) (subst[( λ X → ⊢ v₂ ⦂ X )] ZZ ε₂) ⟨ r₃ , r₆ ⟩
... | OMG rewrite L0-1 Σ′
    | L0-3 ((Σ′ ⨰ Σ₁) +[qty] ⟨ 0 ⟩)
    | ZZ | ◇ (L4 Σ₁ (mapⱽ (_×[qty]_ sx†₁) Σ₂) Σ′)
    | L0-6 sx†₁ Σ₂ Σ′ | L5 Σ₁ Σ′
    | L0-3 ((Σ′ ⨰ Σ′₁) +[qty] ⟨ 0 ⟩) | L0-3 (Σ′ ⨰ Σ′₁) =  OMG -- OMG
